...
app.post("/create-item", isAuth, async (req, res) => {
  ...
  //data validation
  if (!todoText) return res.send({ status: 400, message: "Missing todo text" });
  if (typeof todoText !== "string")
    return res.send({ status: 400, message: "Todo is not a text" });
  if (todoText.length < 3 || todoText.length > 100)
    return res.send({ status: 400, message: "Todo length should be 3-100" });
...
app.post("/edit-item", isAuth, async (req, res) => {
  ...
  try {
    const todoDb = await todoModel.findOne({ _id: blogId });
    console.log(todoDb);
